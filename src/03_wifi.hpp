...
const uint16_t wifiReconnectTimer = 3000; // Delay to reconnect to Wifi after failed
  ...
  const char* esp_ip = "192.168.1.214";
  const IPAddress hostIP(192, 168, 1, 128);   // Had coded Node-red server IP    
  // WiFi.mode(WIFI_STA);  // Not required (default mode is WIFI_STA)
#endif
